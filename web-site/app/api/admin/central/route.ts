...
export async function GET(req:Request){return withActor(req,async()=>{
 ...
 return privateJson({...centralSummary(requests,byId,hospitals.rows,catalog),updatedAt:new Date().toISOString(),banks:[{label:"협회 중앙회 계좌",bank:"우리은행",account:"1005-804-669803",holder:"대한노인돌봄서비스협회(중앙회)"}]});
 },true);}