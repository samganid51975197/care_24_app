...
export default function Home({hospital}:{hospital?:{id:string;name:string;region:string;city:string;type?:string;kind?:string;address?:string;phone?:string;source?:string;sourceLabel?:string;directoryVerified?:boolean;rehabDesignation?:boolean;designationPeriod?:string;addressPending?:boolean}}) {
      ...
      {homeOpen && <section className="home-screen" aria-label="통합간병 업무 홈">
        <div className="home-screen-inner">{completionNotice&&<div className="document-completion" role="status"><span>{completionNotice}</span><Button type="button" variant="outline" onClick={()=>setCompletionNotice("")}>확인</Button></div>}
          {homeStep === "buildings" ? <>
            ...
            <section className="care-entry-actions" aria-label="간병 의뢰와 간병인 신청"><div className="patient-entry"><h2>환자·보호자용</h2><p>편한 의뢰 방법을 선택하세요.</p><div className="patient-entry-buttons"><Button type="button" onClick={openCareRequest}><HeartHandshake aria-hidden="true"/><strong>간병 의뢰</strong><small>의뢰서 작성</small></Button><a href="tel:16005197" aria-label="전화 의뢰, 전국 대표번호 1600-5197"><Phone aria-hidden="true"/><strong>전화 의뢰</strong><small>1600-5197</small></a></div></div><div className="caregiver-entry"><h2>간병인용</h2><p>간병 일을 신청하세요.</p><CaregiverGuide hospitalId={hospital?.id||"snubh"} hospitalName={hospitalName} onRegister={() => moveTo("forms", "care-request")} onRequests={() => {window.setTimeout(()=>document.getElementById("live-matching")?.scrollIntoView({behavior:"smooth",block:"start"}),100)}}/><HospitalDirections name={hospitalName} address={hospital?.address} region={hospital?.region} city={hospital?.city}/></div></section>
            <PaymentNotices hospitalId={hospital?.id||"snubh"}/>{privateAdmin&&<Contribution hospitalId={hospital?.id||"snubh"} />}{isSamsung&&<SamsungIntroduction hospitalId={hospital!.id}/>}{hospital?.id==="seocho-prime"&&<SeochoPrimeIntroduction onCareRequest={openCareRequest}/>}{hospital&&hospital.id!=="seocho-prime"&&!isSamsung&&<section className="card" style={{margin:"20px 0"}}><h2>병원 소개·찾아오는 길</h2><p>{hospitalName} · {hospital.region} {hospital.city} · {hospital.type||hospital.kind}</p>{hospital.address&&<p>{hospital.address}</p>}{hospital.directoryVerified&&<><p>{hospital.rehabDesignation?`제3기 지정 재활의료기관 · ${hospital.designationPeriod}`:"건강보험심사평가원 요양병원 현황에 수록된 기관입니다."}</p>{hospital.phone&&<p>병원 대표전화: <a href={"tel:"+hospital.phone}>{hospital.phone}</a></p>}{hospital.addressPending&&<p>공식 지정 명단의 병원명입니다. 최신 명칭·주소 확인 후 방문해 주세요.</p>}{hospital.source&&<p><a href={hospital.source}>{hospital.sourceLabel||"공식 자료 확인"}</a></p>}</>}{hospital.id==="boramae"?<BoramaeDirections/>:<a href={"https://map.naver.com/p/search/"+encodeURIComponent(hospital.address||`${hospital.region} ${hospital.city} ${hospitalName}`)} target="_blank" rel="noopener noreferrer">지도·대중교통 경로 확인 ↗</a>}</section>}
