...
export default function HospitalDirections({name,address,region,city}:{name:string;address?:string;region?:string;city?:string}) {
 ...
 return <section aria-labelledby="hospital-directions-title" className={styles.card}>
  ...
  <div className={styles.links}>
   <a className={styles.naver} href={'https://map.naver.com/p/search/'+query} target="_blank" rel="noopener noreferrer">네이버지도 길찾기<ArrowUpRight size={22} aria-hidden="true"/></a>
   <a className={styles.kakao} href={'https://map.kakao.com/link/search/'+query} target="_blank" rel="noopener noreferrer">카카오맵 길찾기<ArrowUpRight size={22} aria-hidden="true"/></a>
  </div>
  <p className={styles.help}>지도에서 병원 선택 → 도착·길찾기 → 출발지 입력 → 대중교통 선택</p>
  {bundang&&<a className={styles.official} href="https://www.snubh.org/index.do" target="_blank" rel="noopener noreferrer">병원 공식 안내 ↗</a>}
 </section>;
}