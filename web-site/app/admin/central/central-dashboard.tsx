...
export default function Central(){
 ...
 <section className={styles.payments}><h2>전국 입금·환불 장부</h2><div className={styles.bank}><h3>간병비 입금 계좌</h3>{data.banks.map(bank=><div key={bank.account}><h4>{bank.label}</h4><dl><div><dt>은행</dt><dd>{bank.bank}</dd></div><div><dt>계좌번호</dt><dd>{bank.account}</dd></div><div><dt>예금주</dt><dd>{bank.holder}</dd></div></dl></div>)}<p>안내받은 입금 계좌입니다. 실제 입금 여부는 은행 거래내역 확인 후 아래 장부에 등록하세요.</p></div><p>기존 입금 장부에는 병원별 연결 정보가 없어 전국 합계로 표시합니다. 은행 자동 조회가 아닌 관리자가 확인해 등록한 금액입니다.</p><Contribution/></section></>}
 </main>;
}