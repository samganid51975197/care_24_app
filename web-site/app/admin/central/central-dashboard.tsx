...
type Data={banks:{label:string;bank:string;account:string;holder:string}[];totals:Counts;hospitals:(Counts&{id:string;name:string;kind:string})[];updatedAt:string};
...
export default function Central(){
 ...
 if(denied)return <main className={styles.page}><h1>중앙관리</h1><p>관리자만 확인할 수 있습니다.</p></main>;
 return <main className={styles.page}><header className={styles.header}><div><span>관리자 전용 · 전국 통합</span><h1>중앙관리</h1><p>전국 병원·요양병원의 간병 의뢰와 매칭·입금 현황</p></div><a href="/admin">가입 승인·소속 관리 →</a></header>
 ...
 {data&&<><PaymentNotices/><section className={styles.cards} aria-label="전국 매칭 현황">{[['전체 의뢰',data.totals.total],['간병인 모집 중',data.totals.requesting],['매칭 상담 중',data.totals.matching],['매칭 완료',data.totals.matched]].map(([label,value])=><article key={label}><span>{label}</span><strong>{num(Number(value))}건</strong></article>)}</section>
 ...
 <section className={styles.payments}><h2>전국 입금·환불 장부</h2><div className={styles.bank}><h3>간병비 입금 계좌</h3>{data.banks.map(bank=><div key={bank.account}><h4>{bank.label}</h4><dl><div><dt>은행</dt><dd>{bank.bank}</dd></div><div><dt>계좌번호</dt><dd>{bank.account}</dd></div><div><dt>예금주</dt><dd>{bank.holder}</dd></div></dl></div>)}<p>안내받은 입금 계좌입니다. 실제 입금 여부는 은행 거래내역 확인 후 아래 장부에 등록하세요.</p></div><p>병원별 월·연 나눔 금액을 확인합니다. 병원 미지정 과거 입금은 별도로 표시하며 특정 병원에 임의 배분하지 않습니다.</p><Contribution/></section></>}
 </main>;
}