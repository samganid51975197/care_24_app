...
export default function AuthGate({children}:{children:React.ReactNode}) {
 ...
 return <><div className="account-bar"><div className="account-identity"><span className="account-role">{({admin:'관리자',hospital:'병원 담당자',caregiver:'간병인'} as Record<string,string>)[user.role]}</span><span className="account-name" title={user.name}>{user.name}<span className="account-honorific"> 님</span></span></div><nav aria-label="계정 메뉴">{user.role==="admin"&&<a className="account-link central-admin-link" href="/admin/central" aria-current={path==="/admin/central"?"page":undefined}>중앙 간병24 관리</a>}<a className="account-link" href="/hospital-registration">병원별 간병인 등록</a><a className="account-link" href="/" aria-current={path==='/'?'page':undefined}>업무 화면</a><button className="account-logout" type="button" onClick={logout}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M9 4H5v16h4M13 8l4 4-4 4M8 12h13"/></svg>로그아웃</button></nav></div>{error&&<p role="alert">{error}</p>}{children}</>;
}
